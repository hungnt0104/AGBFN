...
def parse_args():
    ...
    parser.add_argument('--ferplus_path', type=str, default='/content/FerPlus/', help='FerPlus dataset path.')
    parser.add_argument('--pretrained', type=str, default="/home/dyt/FER_workspace/Self-Cure-Network-master/src/cvpr2022/experiments_save/AGBN_FERPLUS_res18_pretrain.pth",
...
def run_training(fold, threshold):
    ...
    datapathlist_train = [cfg.ferplus_path  ]
    labelpathlist_train = [cfg.ferplus_label_path ]
    ...
    data_transforms_val = transforms.Compose([
        ...
        transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                 std=[0.229, 0.224, 0.225])]) 
    datapathlist_test = [cfg.ferplus_path  ]
    labelpathlist_test = [cfg.ferplus_label_path ]                                          
    ...
    max_acc = 0
    fvalid = open('Res18FERPLUS_cvprtest' + str(batchsize_global) + '_' + str(fold) + '.txt', 'a+')
    ftrain = open('Res18FERPLUS_cvprtrain' + str(batchsize_global) + '_' + str(fold) + '.txt', 'a+')
    ftrain_log = open('Res18FERPLUS_cvprtrainlog' + str(batchsize_global) + '_' + str(fold) + '.txt', 'a+')
    fvalid_log = open('Res18FERPLUS_cvprtestlog' + str(batchsize_global) + '_' + str(fold) + '.txt', 'a+')
    lragfn = 0.001
    for i in range(1, args.epochs + 1):
        ...
        if acc > 1:
            torch.save({'iter': i,
                            ...
                            os.path.join("Res152AGBN_FERPLUS_epoch"+str(i)+"_acc"+str(acc.cpu().numpy())+ ".pth"))
            print('Model saved.')
            break
        # pdb.set_trace()
        ...
        with torch.no_grad():
            ...
            if max_acc > 0.91:
                torch.save({'iter': i,
                            'model_state_dict': modelfer.state_dict(),
                            'optimizer_state_dict': optimizer.state_dict(),},
                            os.path.join("Res18_FERPLUS_epoch"+str(i)+"_acc"+str(acc)+ ".pth"))
                print('Model saved.')
                break
